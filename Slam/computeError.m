...
switch Fac.type
    case 'absoluteNary'
        % Inspired in the 'absolute' unary factor:
        % A dense factor which relates several variables with a dense covariance
        
        % DESCRIPTION from the 'absolute' factor
        ...
        % This is an attempt to stack variables and jacobians to reproduce
        % the same design.
        
        % Adding ranges of frames and landmarks as a single one
        % Doing concat with the mean value y
        Fac.exp.e = [];
        Fac.err.z = [];
        Fac.err.J1 = [];
        Z_y = [];
        ...
        i = 1;
        for frm = Frm
            Fac.state.r1 = [Fac.state.r1; frm.state.r];
            Fac.exp.e = [Fac.exp.e ; frm.state.x];
            
            [pq, PQ_y, PQ_e]  = frameIncrement(Fac.meas.y(i:i+6), frm.state.x); % err = h(x) (-) y
            [pq_v, Z_pq] = qpose2vpose(pq);
            Fac.err.z = [Fac.err.z ; pq_v];

            Fac.err.J1 = blkdiag(Fac.err.J1, Z_pq * PQ_e * frm.state.M);
            Z_y = blkdiag(Z_y, Z_pq * PQ_y);
            
            i = i+7;
        end

        for lmk = Lmk
            Fac.state.r1 = [Fac.state.r1; lmk.state.r];
            Fac.exp.e = [Fac.exp.e ; lmk.state.x];
            
            Fac.err.z = [Fac.err.z ; lmk.state.x - Fac.meas.y(i:i+2)]; % err = h(x) - y
            Fac.err.J1 = blkdiag(Fac.err.J1, lmk.state.M); % Jac wrt manifold
            Z_y = blkdiag(Z_y, eye(3,3));
            
            i = i+3;
        end
        
        Fac.err.J2        = zeros(size(Fac.err.J1, 1),0);
        Fac.err.Z         = symmetrize(Z_y * Fac.meas.R * Z_y');
        Fac.err.W         = Fac.err.Z^-1;
        Fac.err.W         = Fac.err.W;
%         Fac.err.Wsqrt     = chol(Fac.err.W);
        Fac.err.Wsqrt     = zeros(size(Fac.err.W));
