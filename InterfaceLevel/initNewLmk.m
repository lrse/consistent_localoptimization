...
if ~isempty(meas.y)  % a feature was detected --> initialize it
    ...
    if strcmp(Map.type, 'ekf')
        ...
        % get new Lmk, covariance.
        [P_LL] = getNewLmkGraphCovs( ...
            Sen.frameInMap, ...
            Rob.frame.P, ...
            Sen.frame.P, ...
            L_rf, ...
        ...
        % add to Map and get lmk range in Map
        Lmk(lmk).state.P = P_LL;
        Map.P(Lmk(lmk).state.r,Lmk(lmk).state.r) = P_LL;
    end
...
function [P_LL] = getNewLmkGraphCovs(SenFrameInMap, RobFrameCov, SenFrameCov,...
    L_rf, L_sf, L_obs, L_n, R, N)
...
if SenFrameInMap % if the sensor frame is in the state
    % co- and cross-variance of map variables (robot and eventually sensor)
    P_MM = blkdiag(RobFrameCov,SenFrameCov);
    L_m = [L_rf L_sf] ;
else
    P_MM = RobFrameCov;
    L_m = L_rf ;
