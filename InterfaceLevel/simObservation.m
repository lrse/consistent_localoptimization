...
% checking landmark wrt sensor for sensor range visibility
[RSf, RS_r, RS_s] = composeFrames(SimRob.frame,SimSen.frame);

% Raw points in sensor frame
if(~isempty(SimLmk.points.coord))
    lrs = toFrame(RSf, SimLmk.points.coord);
    n = diag(vecnorm(lrs));
    n = find(n < SimSen.range);
    SimLmk.points.coord = SimLmk.points.coord(:, n);
    SimLmk.points.id = SimLmk.points.id(:, n);
end
% Raw segments in sensor frame
if(~isempty(SimLmk.segments.coord))
    lrs = toFrame(RSf, SimLmk.segments.coord);
    n = diag(vecnorm(lrs));
    n = find(n < SimSen.range);
    SimLmk.segments.coord = SimLmk.segments.coord(:, n);
    SimLmk.segments.id = SimLmk.segments.id(:, n);
end
