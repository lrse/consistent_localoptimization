...
Time = struct(...
  ...
  'lastFrame',            2400);          % last frame # 2400 for quadraSquare and simpleReverseSquare

randomSeed = 209065;
rng(randomSeed);

% Simulated world and movements
%   - Simulation landmark sets, playground dimensions
%   - Select a scene between: 'simpleSquare', 'quadraSquare', 'simpleReverseSquare'
scene = 'simpleReverseSquare';

% Defines variables World, position, orientationDegrees, dxSteps, daSteps, 
%                   mapMinX, mapMaxX, mapMinY, mapMaxY, mapSizeZ
selectScene;

% Logging flag and parameters
logging = false;
logAllMapStates = false;
...
Robot{1} = struct(...                  % ODOMETRY EXAMPLE
  ...
  'position',           position,...     % robot position in map
  'orientationDegrees', orientationDegrees,...  % orientation, in degrees, [roll; pitch; yaw].
...
%   'sensingArea',        3,...           % sensor sensing area range [m]
...
Sensor{1} = struct(...
  ...
  'positionStd',        [0;0;0],...     % position error std
  'orientationStd',     [0;0;0],...     % orient. error std
  'sensingArea',        10,...           % sensor sensing area range [m]
  'imageSize',          [640;480],...   % image size
...
Opt = struct(...
  'map',              struct(...     % options for the map
    ...
    'localWindowSize', 5,...        % window optimitation size in case of local graph 
    'numLmks',        1000,...       % number of 3d landmarks
    'lmkSize',        3,...          % Size of landmark state
    'lmkDSize',       3,...          % Size of lmk error state
    'numFrames',      500,...        % number of frames in graph
  ...
  'solver',           struct(...     % graph solver
    ...
    'target_dres',    1e-8,...       % exit criterion for residual variation
    'target_res',     1e-9),...      % exit criterion for current residual
  ...
  'marginal',         struct(...     % marginalization options 
...
SimOpt = struct(...                    
  'random',           struct(...      % random generator options
    ...
    'fixedSeed',      randomSeed,...       % random seed for non-random runs
    'seed',           []),...          % current seed
...
FigOpt = struct(...
  ...
  'map',            struct(...      % map figure options
    ...
    'lims',         struct(...      % playground limits
      'xMin',            mapMinX,... % mapSize defined according to the scene selected             
      'xMax',            mapMaxX,...
      'yMin',            mapMinY,...
      'yMax',            mapMaxY,...
      'zMin',            -mapSizeZ,...
